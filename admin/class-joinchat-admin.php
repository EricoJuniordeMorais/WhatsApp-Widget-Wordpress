...
class JoinChatAdmin {
		...
		if ( 'general' == $tab ) {
			...
			$sections = array(
				...
				'chat'      => array(
					...
					'form_text'     => __( 'Form Text', 'creame-whatsapp-me' ),
	...
	public function settings_validate( $input ) {
		...
		$input['form_text']     = in_array( $input['form_text'], array( 'no', 'yes', 'auto' ) ) ? $input['form_text'] : 'no';
	...
	public function field_output( $field_id ) {
		...
		if ( strpos( $field_id, 'view__' ) === 0 ) {
			...
			switch ( $field_id ) {
				...
				case 'message_start':
					$output = '<input id="joinchat_message_start" name="joinchat[message_start]" value="' . $value . '" type="text" maxlength="20" class="regular-text" placeholder="' . esc_attr__( 'Enviar Mensagem', 'creame-whatsapp-me' ) . '"> ' .
						'<p class="description">' . __( 'Text to Enviar Mensagem on Chat Window button', 'creame-whatsapp-me' ) . '</p>';
					break;
				...
				case 'dark_mode':
					...
					case 'form_text':
						$output = '<fieldset><legend class="screen-reader-text"><span>' . __( 'Form Text', 'creame-whatsapp-me' ) . '</span></legend>' .
							'<label><input name="joinchat[form_text]" value="no" type="radio"' . checked( 'no', $value, false ) . '> ' .
							__( 'No', 'creame-whatsapp-me' ) . '</label><br>' .
							'<label><input name="joinchat[form_text]" value="yes" type="radio"' . checked( 'yes', $value, false ) . '> ' .
							__( 'Yes', 'creame-whatsapp-me' ) . '</label><br>' ;
						break;
